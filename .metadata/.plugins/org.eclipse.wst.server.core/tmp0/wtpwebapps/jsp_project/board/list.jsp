...
<body>
	...
	<div>
		...
		<a href="/brd/list"><button type="button">검색 해지</button></a>
	</div>
	<table>
		<tr>
			<th>no.</th>
	...
	<div>
		<c:if test="${ph.prev}">
			<a
				href="list?pageNo=${ph.startPage - 1}&qty=${ph.pgvo.qty}&type=${ph.pgvo.type}&keyword=${ph.pgvo.keyword}">
				◀ </a>
		</c:if>
		<c:forEach begin="${ph.startPage}" end="${ph.endPage}" var="i">
			<a
				href="list?pageNo=${i}&qty=${ph.pgvo.qty}&type=${ph.pgvo.type}&keyword=${ph.pgvo.keyword}">${i}</a>
		</c:forEach>
		<c:if test="${ph.next}">
			<a
				href="list?pageNo=${ph.endPage + 1}&qty=${ph.pgvo.qty}&type=${ph.pgvo.type}&keyword=${ph.pgvo.keyword}">
				▶ </a>
		</c:if>
	</div>
