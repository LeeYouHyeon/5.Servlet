...
public class MemberController extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		case "insert":
			try {
				// 아이디 중복체크를 했다고 가정
				MemberVO mvo = new MemberVO(request.getParameter("id"), request.getParameter("pwd"),
		...
		case "login":
			try {
				/*
				 * 1. jsp에서 보내온 id, pwd 받기 2. id, pwd로 객체를 생성 => DB로 전송 3. 해당 ID와 PWD로 객체를 검색해서
				 * 객체 형태로 리턴 4. 객체를 컨트롤러로 가져와서 세션에 저장 session : 모든 jsp에서 공유하는 객체 / 브라우저가 종료되면 삭제
				 */
				String id = request.getParameter("id");
		...
		case "list":
			try {
				...
				int totalCount = msv.getTotal(pgvo);
		...
		case "modify":
			try {
				...
				destPage = mvo == null ? "/index.jsp" : "/member/modify.jsp";
			} catch (Exception e) {
				// TODO: handle exception
				e.printStackTrace();
			}
		...
		case "remove":
			try {
				HttpSession session = request.getSession();
				String id = ((MemberVO)session.getAttribute("ses")).getId();
				isOk = msv.delete(id);
